...
function create() {
    ...
    this.title1 = this.add.text(110, 30, 'Happy Mrs. Chicken', menuFont).setOrigin(0.5);
    ...
    this.drawScoreBox = (width) => {
        ...
        this.scoreBg.fillRect(Math.floor(x), Math.floor(y), Math.floor(width + 2), Math.floor(boxHeight));

        this.scoreBg.fillStyle(borderColor, 1);
        this.scoreBg.fillRect(Math.floor(x), Math.floor(y + boxHeight - 1), Math.floor(width + 2), 1);
        this.scoreBg.fillRect(Math.floor(x), Math.floor(y), 1, Math.floor(boxHeight));

        this.scoreBg.setVisible(true);
    };

    // Score text
    this.scoreText = this.add.text(WIDTH - 2, -1, '0', scoreFont)
        .setOrigin(1, 0)
        .setDepth(5)
        .setScrollFactor(0)
        .setVisible(false);
        ...
        callback: () => {
            if (!this.menu) {
                this.eggs.getChildren().forEach(egg => {
                    ...
                    const hatchAnim = scene.add.sprite(x, y, 'hatching')
                        .setDepth(2)
                        .play('hatch');
    ...
    // Loading sprite
    this.loadingSprite = this.add.sprite(WIDTH - 12, HEIGHT - 12, 'egg')
        .setVisible(false)
        .setDepth(10);
    ...
    scene.time.delayedCall(400, () => {
        ...
        scene.time.delayedCall(500 * steps, () => {
            ...
            scene.time.delayedCall(400, () => {
                ...
                const main = scene.add.sprite(WIDTH / 2, HEIGHT / 2, 'chicken').setDepth(2);
...
Phaser.Scene.prototype.handleA = function () {
    if (this.menu) {
        if (this.selectedButton === 0 && !this.loading) {
            ...
            const msg = this.add.text(WIDTH / 2, HEIGHT / 2, 'Created by Sonicblaston', textFont)
                .setOrigin(0.5)
                .setDepth(999);
